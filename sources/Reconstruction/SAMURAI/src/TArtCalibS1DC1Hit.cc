...
TArtCalibS1DC1Hit::TArtCalibS1DC1Hit(const TString outdataname) 
  : TArtCalibDCHit(outdataname, S1MWDC)
{
  TArtCore::Info(__FILE__,"Creating the S1-MWDC #1 detector hit objects...");
}

//__________________________________________________________
TArtCalibS1DC1Hit::~TArtCalibS1DC1Hit()  {
}

//__________________________________________________________
void TArtCalibS1DC1Hit::LoadData(TArtRawSegmentObject *seg)   {
  TArtCore::Debug(__FILE__,"LoadData S1-MWDC #1");
  ...
  for(Int_t j=0;j<seg->GetNumData();j++){
    ...
    TArtDCHitPara *para = (TArtDCHitPara*)setup->FindS1DC1HitPara(&mm);
    if(NULL == para){
