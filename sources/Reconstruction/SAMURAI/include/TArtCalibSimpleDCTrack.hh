...
You can choose which you want to use by changing TArtCalibSimpleDCTrack::ReconstructData method.
...
1. Fill the hit data array with TArtCalibSimpleDCTrack::SetHitsBuffer. All of DC hits will be stored in the vector of hit array for each direction (X, Y, U or V).
2. For each direction, 1D track will be calculated with TArtCalibSimpleDCTrack::Calc1DTrack. It means hits on X direction is not connected to hits on U and V direction. This procedure is mainly to throw away the hits which is not coming from real hits.
3. All of the 1D tracks for each direction will be sorted based on chi2 value and only some, which is defined as "ntrack_keep in TArtCalibSimpleDCTrack::RecoTrack", candidates will be kept for connecting the hits on X, U and V direction. 
4. These candidates will be connected with TArtCalibSimpleDCTrack::CalcTrackWith1DTracks method, which will return the track data container. This data container will be store in TClonesArray of TArtDCTrack after sorting based on chi2.
...
#ifndef _TARTCALIBSIMPLEDCTRACK_H_
#define _TARTCALIBSIMPLEDCTRACK_H_
...
class TArtCalibSimpleDCTrack : public TArtReconstruction {

 public:

  TArtCalibSimpleDCTrack(const TString indata, 
  ...
  virtual ~TArtCalibSimpleDCTrack();
 ...
 protected:
  ...
  void RecoTrack();
  void CalcTrack(std::vector <TArtDCHit *> *hits, TString dir,Int_t numlayer, TObjArray *trs);
 ...
 private:
  ...
  ClassDef(TArtCalibSimpleDCTrack,1)
