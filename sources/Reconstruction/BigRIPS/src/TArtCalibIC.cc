...
void TArtCalibIC::LoadData()   {
  ...
  for(Int_t i=0;i<fEvent->GetNumSeg();i++){
    ...
    if(ICE == detector || ICT == detector || ICB == detector || ICF == detector) LoadData(seg);
  }
...
void TArtCalibIC::LoadData(TArtRawSegmentObject *seg)   {
  ...
  Int_t fpl      = seg->GetFP();
  Int_t detector = seg->GetDetector();
  if(!(ICE == detector || ICT == detector || ICB == detector || ICF == detector)) return;
  ...
  for(Int_t j=0;j<seg->GetNumData();j++){
    ...
    // set raw data
    if (ICT == detector) {
      Int_t ic_ch = para->FindCh_t(&mm);
      if(ic->GetRawTDC(ic_ch)<=0) {//use first TDC value (when it is multihit)
	ic->SetRawTDC(ic_ch,val);
      }
    } else {
